...
public class innerportalblock extends BlockBreakable {
    ...
    public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
        byte b0 = 0;
        byte b1 = 0;
        if (par1World.getBlockId(par2 - 1, par3, par4) == config.frame || par1World.getBlockId(par2 + 1, par3, par4) == config.frame) {
            b0 = 1;
        }
        if (par1World.getBlockId(par2, par3, par4 - 1) == config.frame || par1World.getBlockId(par2, par3, par4 + 1) == config.frame) {
            b1 = 1;
        }
        if (b0 == b1) {
            return false;
            ...
            for (l = 0; l < 2; ++l) {
                for (i1 = 0; i1 < 3; ++i1) {
                    par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, config.portal, 0, 2);
                }
            }
            return true;
        }
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity) {
        if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP))) {
            ...
            } else if (thePlayer.dimension != config.dimension) {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, config.dimension, new Portal(thePlayer.mcServer.worldServerForDimension(config.dimension)));
            } else {
                thePlayer.timeUntilPortal = 10;
